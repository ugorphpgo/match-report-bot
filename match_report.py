...
# списка локали, решает скоринг, а не список лиг.
# ---------------------------------------------------------------------------
CARRY_OVER_CONFIG = {
...
def fetch_carry_over_matches(day_str, max_hour):
    ...
    получает DEFAULT_PRIORITY и до списка не доходит.
    """
    try:
...
# Сколько матчей хочется в каждом виджете. Это цель, а не потолок: турнир
# между виджетами не делится и на хвосте не обрезается (см. _widget_end).
TOP_EVENTS_SIZE = 14
TOP_MATCHES_SIZE = 12


def _widget_end(ranked, start, target):
    """Где кончается виджет, который начинается с ranked[start].

    Берём турниры целиком, пока граница приближается к target: граница
    проходит между турнирами там, где итог ближе к цели, при равенстве — с
    турниром (сильный турнир не спускается в слабый виджет). Первый турнир
    берётся всегда, каким бы большим он ни был, — иначе отбор ЧМ на 30 матчей
    ...
    while end < len(ranked):
        league_id = ranked[end]["league"]["id"]
        block_end = end
        while block_end < len(ranked) and ranked[block_end]["league"]["id"] == league_id:
            block_end += 1
        taken, with_block = end - start, block_end - start
        if taken and abs(with_block - target) > abs(taken - target):
            break
        end = block_end
    return end
...
def split_widgets(ranked):
    """Делит список локали на две пачки под виджеты админки:
    (top_matches, top_events). Топ ивенты — сильнейшие турниры списка, топ
    матчи — следующие за ними; граница — между турнирами (_widget_end).
    ...
    ranked — выход rank_matches, уже обрезанный: повторное деление даёт ту
    же границу, потому что каждый турнир в нём либо взят, либо нет.
    """
    events_end = _widget_end(ranked, 0, TOP_EVENTS_SIZE)
    return ranked[events_end:], ranked[:events_end]
...
def build_locale_message(locale_key, cfg, matches, report_date, display_date):
    ...
    список, а локали — матчи СВОИХ турниров (local_leagues)
    ...
    # локалей, в общий список такие матчи не попадают вовсе.
